...
function ReadingContent() {
  ...
  // 🔥 FIX: Load data WITHOUT checking phonics
  useEffect(() => {
  ...
  // ... rest of the file (timer, detectStumbles, startListening, etc.) remains the same ...

  // 🔥 startListening - removed phonics check
  ...
  // ... rest of the functions (transcribeAudio, stopListening, endSession, etc.) ...

  // 🔥 Celebration screen - shows reading stats ONLY if child actually read
  if (isComplete) {
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-6" style={{ background: currentTheme?.background || '#f7f2eb' }}>
        <div className="bg-white rounded-3xl shadow-xl p-12 max-w-2xl w-full text-center">
          ...
          <button onClick={() => router.push('/dashboard')} className="px-8 py-3 bg-[#1e1916] text-white rounded-full font-medium hover:shadow-xl transition-all">📊 Back to Dashboard</button>
  ...
  // ... rest of the file (questions, main reading screen) ...

  // Return the main reading screen
  return (
